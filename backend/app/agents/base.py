...
class BaseAgent(ABC):
    ...
    def build_tools_prompt_section(self) -> str:
        ...
        lines = [
            ...
            "### 什么时候使用工具",
            "",
            "• 需要查询外部信息、获取实时数据时 → 调用查询类工具",
            "• 需要执行具体操作（如文件操作、API调用）时 → 调用操作类工具",
            "• 任务需要的信息不在你的知识范围内时 → 主动搜索/调用工具",
            "• 不确定答案时 → 先用工具查证再回答",
            "",
            "### 工具调用格式",
            "",
            '使用 `<tool_call>` 标签包裹 JSON 格式的工具调用，严格按照以下格式：',
            "",
            '<tool_call>',
            '{',
            '  "name": "工具名称",',
            ...
            "• 每次只能调用一个工具",
            "• 必须使用正确的工具名称和参数",
            "• 调用后等待工具返回结果，再根据结果继续思考",
            "• 如果工具结果不足以回答，可以继续调用其他工具或同一个工具",
            "• 当你有了最终答案时，直接回答，不再使用 `<tool_call>` 标签",
            "",
            "### 工具列表",
            "",
        ]
